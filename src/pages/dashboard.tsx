...
import { getCredits } from 'apiSdk/credits';
import { CreditInterface } from 'interfaces/credit';
import { getTasks } from 'apiSdk/tasks';
import { TaskInterface } from 'interfaces/task';

import { ScreenshotListPage as ShowCaseList } from 'pages/screenshots';
...
function DashboardPage() {
  ...
  const creditStat = useSWR<PaginatedInterface<CreditInterface>>(
    () => `/credits`,
    () => getCredits({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Credits',
      result: creditStat?.data?.totalCount,
      isLoading: creditStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tasks',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`screenshots`} passHref legacyBehavior>
                  <Button

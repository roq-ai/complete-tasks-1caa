...
function CreditCreatePage() {
  ...
  const formik = useFormik<CreditInterface>({
    initialValues: {
      amount: 0,
      credit_date: new Date(new Date().toDateString()),
      status: '',
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="credit_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Credit Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.credit_date ? new Date(formik.values?.credit_date) : null}
              onChange={(value: Date) => formik.setFieldValue('credit_date', value)}
            />
          </FormControl>
